				...
				<!-- 상단: 프로필, ...더보기 버튼 (삭제,수정) -->
					...
					<!-- 프로필 : 누르면 마이페이지로 이동한다. -->
						<a href="#">
							<div class="userProfileId d-flex ml-2">
								<label class="mr-2"><img src="${contentView.user.profileImageUrl}" alt="프로필 사진"
									width="40px" height="40px"></label>
								<h4 class="m-1 text-dark">${content.user.name}hi1856</h4>
							</div>
						</a>

					<!-- modal: 글쓴 사용자와 로그인 사용자가 일치할 때만 삭제 가능 -->
					<c:if test="${userName eq content.user.name}">
						<a href="#" data-toggle="modal" class="moreBtn"
							data-target="#MoreModal" data-post-id="${content.post.id}">
						<img src="/staticImages/moreIcon.png" id="feedHeaderMoreImg"
							alt="더보기" width="25px" class="mr-3" height="25px">
						</a>
					</c:if>
					</div>
					...
					<!-- 하트 -->
						<div class="ml-2 mr-2">
							<a href="#" class="likeBtn"> 
								...
								<!-- 좋아요 상태 --> 
								<img src="/staticImages/pinkHeart.png" id="basicHeartImg"
									class="d-none" width="35px" height="35px" alt="핑크하트">
							</a> 
							<a href="#"> 
					...
					<!-- 댓글 목록  -->
						<c:forEach items="${content.commentList}" var="commentView">
							<div class="ml-5 mt-2 d-flex border-left justify-content-between">
								<div>
									<span class="font-weight-bold mx-3">${commentView.user.name}gg</span>
									<span>${commentView.comment.content}gkgkfhfhf</span>
								</div>
								<!-- 댓글쓴이가 본인이면 삭제버튼 노출 -->
								<a hrdf="#" class="commentDelBtn mr-5" data-comment-id="${comment.id}"> 
					...
					<c:if test="${not empty userId}">
						<div class="comment_write  mt-2 d-flex ml-2">
							<!-- 댓글쓴이 프로필,  -->
							<div class="mr-3">
								<img src="${commentView.user.profileImageUrl}" alt="사용자"
									id="profile" data-comment-userLoginid="${commentView.user.loginId}"
							...
							<!-- 댓글 달기 부분 -->
							<div class="ml-3">
								<button type="button" class="commentBtn btn d-none"
